...
# tu jest dialogflow, nie jest na razie ta funkcja nigdzie uzywana 
def detect_intent_audio_dialogflow(project_id, session_id, audio_file_path, language_code, cred_file):
    ...
    print ('detect_intent_from_audio')
...
def callback_common(response, sound_file, playback_queue):
    ...
    print("parameters:")
    print (u'{}').format( response.query_result.parameters )

    # print("response_tpye:")
    # print(type(response))
    # new_response = pf.json_format.MessageToJson(response.query_result, including_default_value_fields=False)
    # print("new_response")
    # print(new_response)
    # print("new response type")
    # print(type(new_response))

    # json_acceptable_str = new_response.replace("'", "\"")
    # new_response = json.loads(json_acceptable_str)
    ...
    for param_name, param in response.query_result.parameters.fields.iteritems():
        ...
        print(type(param))
        if param.list_value:
            print("has list_value")
            print(param.list_value)
            if hasattr(param.list_value, "values"):
                print("has values inside")
                print(param.list_value.values)
                print("type'o")
                print(type(param.list_value.values))

                param = str(param.list_value.values)[1:-1]
                print("after strip")
                print(param)
                print("type'o stripped")
                print(type(param))

        # else:
        #     if hasattr(param.list_value, "values"):
        #         print("has values")
        #         new_param = param.values

        print("param after changes:")
        print(param)
...
def callback_wav(data, agent_name, playback_queue, cred_file):
    ...
    #Dialogflow version:
    response, sound_file = detect_intent_audio_dialogflow(agent_name, "test_sess_012", data.data, "pl", cred_file)
...
def listener():
    ...
    say_as = SaySentenceActionServer( 'rico_says', playback_queue, odm, sentence_dict, 'text-to-speech-qtruau', cred_file_incare_dialog, sc)
